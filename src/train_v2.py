"""
CoT reasoning distillation trainer v2.
Improvements:
  1. max_length defaults to 2048 (covers 95%+ of CoT chains)
  2. Training prompt uses the chat template (consistent with evaluation)
  3. Expanded LoRA modules: q/k/v/o + gate/up/down_proj, rank=32
  4. Supports filtered CoT data
Integrated with SwanLab experiment tracking + resume support
"""
import os
import re
import sys
...
# ============================================================
# Dataset: the training prompt uses the chat template, aligned with evaluation
# ============================================================

class CoTDistillDataset(Dataset):
    """
    v2 improvement: build the prompt with apply_chat_template, consistent with the evaluation stage.
    """
    ...
    def _format_input(self, item: dict) -> str:
        """v2: build the prompt with the chat template, consistent with eval.py's build_prompt."""
        q = item["question"]
    ...
    def __getitem__(self, idx):
        ...
        # Concatenate input + CoT as the complete training sequence
...
class JointDistillationLoss(nn.Module):
    """CoT sequence distillation loss."""
    def __init__(self):
        super().__init__()
...
# ============================================================
# Model loading: v2 expanded LoRA modules + rank=32
# ============================================================

def load_model_and_tokenizer(model_name: str, lora_config: dict, apply_lora: bool = True):
    """v2: LoRA covers q/k/v/o + gate/up/down_proj, rank=32."""
    ...
    # v2: expanded LoRA target modules
    lora_cfg = LoraConfig(
...
class CoTDistillTrainer:
    ...
    def evaluate_qa(self):
        """QA accuracy evaluation (batch inference, significantly faster)."""
        ...
        self.model.eval()
        self.tokenizer.padding_side = "left"  # CausalLM batch generation requires left padding
        correct = 0
        total = 0
        t0 = time.time()
        ...
        self.tokenizer.padding_side = "right"  # Restore default
        print()
...
def main():
    ...
    parser.add_argument("--eval_max_samples", type=int, default=0,
                        help="Number of evaluation samples, 0 = full evaluation")
    ...
    # Non-resume mode: clean up old checkpoints and start cleanly from epoch_0
    if not args.resume:
        ...
        if output_path.exists():
            ...
            print(f"  [CLEAN] Cleaned up old checkpoints: {args.output_dir}")

    if args.resume:
        ckpt_info = CoTDistillTrainer.find_latest_checkpoint(args.output_dir)
        if ckpt_info is None:
            print(f"[WARNING] --resume specified, but no checkpoint found; starting from scratch.")
        else:
            ...
            if next_epoch >= args.epochs:
                print(f"[INFO] All {args.epochs} epochs are complete.")
                return
            print(f"[RESUME] Resuming from checkpoint:")
            print(f"  Checkpoint: {ckpt_info['ckpt_path']}")
            print(f"  Completed epochs: {resume_epoch}/{args.epochs - 1}")
            print(f"  Completed global steps: {resume_global_step}")
            print(f"  Will resume training from epoch {next_epoch}")
